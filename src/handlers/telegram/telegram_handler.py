...
class TelegramHandler:
    ...
    async def _handle_report(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        Format: /report <recipient_name> <YYYY-MM-DD>
        """
        try:
            # Get the device ID (user ID in Telegram)
            device_id = str(update.effective_user.id)
            ...
            if not context.args or len(context.args) != 2:
                await update.message.reply_text(
                    "❌ Invalid command format.\n"
                    "Usage: /report <recipient_name> <YYYY-MM-DD>\n"
                    "Example: /report Child1 2026-06-20"
                )
                return
            ...
            # Convert the requested date to the team's timezone
            # User asks for "2026-06-20" meaning "all evaluations on June 20 in their timezone"
            try:
            ...
            if not evaluations:
                await update.message.reply_text(
                    f"📊 No evaluations found for {recipient.name_en} on {date_str}"
                )
                return
            ...
            logger.info(
                ...
                f"for {recipient.name_en} on {date_str}"
            )
